...
const dundasMarketGlb = await gltfLoader.loadAsync(
  `${import.meta.env.BASE_URL}Day 9 - DundasMarketSign.glb`
);
const dundasMarket = dundasMarketGlb.scene;

dundasMarket.traverse((child) => {
  if (child.isMesh) {
...
dundasMarket.updateMatrixWorld(true);

// compute bounds and scale so the model's largest dimension equals `targetSize`
let box = new THREE.Box3().setFromObject(dundasMarket);
const size = box.getSize(new THREE.Vector3());
const maxDim = Math.max(size.x, size.y, size.z);
const targetSize = 4; // world units you want the model to fit in
if (maxDim > 0) {
  const scale = targetSize / maxDim;
  dundasMarket.scale.setScalar(scale);
  dundasMarket.updateMatrixWorld(true); // update after scaling
}

// recompute bounds and get center
box = new THREE.Box3().setFromObject(dundasMarket);
const center = box.getCenter(new THREE.Vector3());

// create a pivot at the world origin and add the model offset so its center is at pivot
const pivot = new THREE.Group();
scene.add(pivot);
dundasMarket.position.sub(center); // move model so its center is at (0,0,0) relative to pivot
